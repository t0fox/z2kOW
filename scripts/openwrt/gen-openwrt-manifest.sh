...
MODE="gen"
SRC=""; TREE=""; REF=""; APIMIN="1"; OUT=""; ALLOW_DIRTY=0
CHANGED=""
while [ $# -gt 0 ]; do
    case "$1" in
...
python3 - "$SRC" "$TREE" "$REF" "$APIMIN" "$OUT" "$_pkg_exact" "$_pkg_pref" <<'PYEOF'
import json, sys, hashlib, os, subprocess
src, tree, ref, apimin, out, pkgexact, pkgpref = sys.argv[1:8]
...
shas = sorted(man['files_sha256'].items())
...
sys.stderr.write('openwrt manifest: current=%s ref=%s api_min=%s%s keys=%d dropped_keenetic=%d\n'
