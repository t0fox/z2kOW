...
generate_nfqws2_opt_from_strategies() {
    ...
    # `send` + `drop` are tagged on every circular arm: circular skips untagged
    # actions. They replay the real discovery/STUN request after its decoys and
    # prevent the queued copy from disappearing at the postrouting NFQUEUE hook.
    ...
    # PLATFORM-NEUTRAL HOOK (allowlisted): дополнительный exclude-файл
    # фичи (путь — из окружения, не из платформы: любая платформа может
    # выставить; unset/отсутствует = поведение 1-в-1, Keenetic не затронут).
    # Мотивация — владение: feature-owned пины (напр. RT-мосту нужны его 5
    # доменов вне desync) нельзя дописывать в user-owned whitelist.txt —
    # их некому снять, и чужой teardown либо трогает пользовательские строки,
    # либо оставляет вечное исключение прямого трафика. Отдельный файл
    # решает обе стороны: truncate/rm — только свой файл, чужого не касаемся.
    if [ -n "${Z2K_HOSTLIST_EXCLUDE_EXTRA:-}" ] && [ -f "$Z2K_HOSTLIST_EXCLUDE_EXTRA" ]; then
        wl_excl="$wl_excl --hostlist-exclude=$Z2K_HOSTLIST_EXCLUDE_EXTRA"
    fi
