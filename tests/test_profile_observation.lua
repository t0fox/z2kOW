...
H.test('YouTube TCP subdomains share rotation while ads and other pools stay separate',function()
    ...
    for _,host in ipairs({'www.youtube.com','m.youtube.com','img.youtube.com','accounts.youtube.com'}) do
        ...
        H.eq('youtube.com|4',key)
        H.eq(root,rec)
    end
    H.eq('youtube.com|4',root_key)
    local ads_key,ads=record('ads.youtube.com')
    H.eq('ads.youtube.com|4',ads_key)
    assert(ads~=root,'ads failure must not rotate the YouTube family')
...
H.test('a frozen www YouTube TCP strategy migrates to the shared row once',function()
    fresh_state()
    local f=assert(io.open(state_path,'w'))
    ...
    f:close()
    local h=H.step(profile_initial('yt_tcp','img.youtube.com'))
    H.eq(2,H.executed); H.eq(2,h.final)
    H.eq(nil,P._state().yt_tcp['youtube.com|4'].sni)
    P.flush()
    H.eq(2,disk_strategy('yt_tcp','youtube.com|4'))
    H.eq(3,disk_strategy('gv_tcp','googlevideo.com|4'))
    -- A newer operator edit to the shared row must not revive the old www pin.
    f=assert(io.open(state_path,'w'))
    f:write('yt_tcp\tyoutube.com|4\t1\t2000\tauto\n')
    f:write('yt_tcp\twww.youtube.com|4\t2\t1000\tfrozen\n')
    f:close()
    P._reset(); autostate={}
    h=H.step(profile_initial('yt_tcp','m.youtube.com'))
    H.eq(1,H.executed); H.eq(nil,h.final)
end)
H.test('an existing frozen YouTube root wins over a conflicting www pin',function()
    fresh_state()
    local f=assert(io.open(state_path,'w'))
...
H.test('a busy state lock delays but does not lose the YouTube migration',function()
    fresh_state()
    local f=assert(io.open(state_path,'w'))
    f:write('yt_tcp\twww.youtube.com|4\t2\t1000\tfrozen\n'); f:close()
    f=assert(io.open(state_path..'.lock','w'))
    f:write('1000'); f:close()
    local h=H.step(profile_initial('yt_tcp','img.youtube.com'))
    H.eq(2,h.final)
    H.eq(nil,disk_strategy('yt_tcp','youtube.com|4'))
    os.remove(state_path..'.lock')
    H.step(profile_initial('yt_tcp','m.youtube.com'))
    H.eq(2,disk_strategy('yt_tcp','youtube.com|4'))
end)
