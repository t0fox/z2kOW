#!/bin/sh
# p-85.4 OpenWrt adapter boundary and provenance contract.
. "$(dirname "$0")/helper.sh"
_t_plan "ow-p854-contract"
...
assert_eq "p-85.4 current" "p-85.4" "$_current"
assert_eq "p-85.4 sequence" "112" "$_seq"
...
if sh "$REPO/scripts/openwrt/gen-openwrt-manifest.sh" \
       --source-manifest "$REPO/UPDATES.json" --tree "$REPO" --ref p-85.4 \
