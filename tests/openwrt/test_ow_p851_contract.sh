#!/bin/sh
# p-85.10 OpenWrt adapter boundary and provenance contract.
. "$(dirname "$0")/helper.sh"
_t_plan "ow-p8510-contract"
...
assert_eq "p-85.10 current" "p-85.10" "$_current"
assert_eq "p-85.10 sequence" "118" "$_seq"
...
if sh "$REPO/scripts/openwrt/gen-openwrt-manifest.sh" \
       --source-manifest "$REPO/UPDATES.json" --tree "$REPO" --ref p-85.10 \
