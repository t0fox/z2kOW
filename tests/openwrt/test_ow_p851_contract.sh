#!/bin/sh
# p-85.8 OpenWrt adapter boundary and provenance contract.
. "$(dirname "$0")/helper.sh"
_t_plan "ow-p858-contract"
...
assert_eq "p-85.8 current" "p-85.8" "$_current"
assert_eq "p-85.8 sequence" "116" "$_seq"
...
if sh "$REPO/scripts/openwrt/gen-openwrt-manifest.sh" \
       --source-manifest "$REPO/UPDATES.json" --tree "$REPO" --ref p-85.8 \
