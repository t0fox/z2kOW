#!/bin/sh
# p-85.1 OpenWrt adapter boundary and provenance contract.
. "$(dirname "$0")/helper.sh"
_t_plan "ow-p851-contract"
...
assert_eq "p-85.1 current" "p-85.1" "$_current"
assert_eq "p-85.1 sequence" "109" "$_seq"
...
if sh "$REPO/scripts/openwrt/gen-openwrt-manifest.sh" \
       --source-manifest "$REPO/UPDATES.json" --tree "$REPO" --ref p-85.1 \
