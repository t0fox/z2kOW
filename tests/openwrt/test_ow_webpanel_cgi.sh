...
# the adapter packages are r26: the panel must report the payload truth and expose
# the package release separately instead of presenting the seed/tag mismatch
# as an installed upstream version.
cat > "$T/bin/apk" <<'EOF'
#!/bin/sh
case "$*" in
    *z2k-adapter*) echo 'z2k-adapter-0.1.0-r26 aarch64_cortex-a53 [installed]' ;;
    *z2k-webpanel*) echo 'z2k-webpanel-0.1.0-r26 aarch64_cortex-a53 [installed]' ;;
...
assert_eq "update: adapter release stays explicit" "z2k-adapter-0.1.0-r26" "$(_jget "$OUT" 'd["adapter_package"]')"
assert_eq "update: webpanel release stays explicit" "z2k-webpanel-0.1.0-r26" "$(_jget "$OUT" 'd["webpanel_package"]')"
