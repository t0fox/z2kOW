...
# туннельные имена RT/WARP (будущие слои, им здесь не место), ndm, /opt-пути.
# TG/HTTP-tunnel из списка УБРАНЫ осознанно (Stage 3 приземлился:
# platform/openwrt/tg.sh + проводка; см. docs/openwrt-telegram-contract.md).
# Границы токенов — чтобы не ловить SHIPPED/tcp16_asn
# (легитимный lua-state plumbing ядра). Когда RT/WARP приземлятся —
...
for _f in "$REPO"/platform/openwrt/*.sh \
    ...
    _h="$(sed 's/#.*$//' "$_f" | grep -v 'Z2K_ZAPRET2_RUNTIME.*:-/opt/zapret2' \
        | grep -inE 'keenetic|S99|(^|[^a-zA-Z])PPE([^a-zA-Z]|$)|watchdog|tcp16-probe|Entware|/opt/|z2k-warp|rt-proxy|(^|[^a-zA-Z_])ndm([^a-zA-Z_]|$)' || true)"
