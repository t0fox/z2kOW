...
if [ -z "$_all" ]; then
    ...
    for _f in $_bad; do
        case "$_f" in
            ...
            tests/test_config_official.sh|tests/test_found_domains_survive_reinstall.sh|tests/test_panel_domain_probe.sh|tests/test_panel_modules_delivered.sh|tests/test_profile_observation.sh|tests/test_quic_pool_general.sh|tests/test_update_sequence_e2e.sh|tests/test_update_jitter.sh|z2k-detect/cmd/z2k-detect/z2k_hostlists.go|z2k-detect/internal/decision/decision.go) continue ;;
            UPDATES.json)
