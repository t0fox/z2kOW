...
# ограничен build-tagged реализацией и Go overlay. p-85.16 health/WG
# семантические backport-ы и YouTube per-host rollback перечислены ниже.
# Иначе — провал с
...
#     internal/engine/engine_test.go + internal/health/health.go +
...
#   tests/test_build_matrix.sh: keep the shipped Keenetic WARP binaries pinned
#     to signed r-85.12 while OpenWrt compiles its adapted runtime in package CI.
#   tests/test_update_content_verification.sh: verify the manifest against the
#     committed release snapshot rather than the post-release working tree.
#   tests/test_warp_games.sh: shared updater regression for advisory-only missing
#     game feeds; it must not turn ordinary list download errors into advisories.
#   z2k-warpd/internal/engine/engine_test.go: exercise external-backend ownership
#     across ready, shutdown, transport cleanup, and transient status removal.
#   z2k-warpd/internal/transport/wg/bind.go: p-85.16 AWG-compatible handshake
#     preamble, shared transport semantics; regression is in bind_test.go.
#   z2k-warpd/internal/domainroute/*_test.go + internal/edgepick/*_test.go:
#     consolidate scenarios that share a Go package and harness; keep the
#     OpenWrt-tagged nft PairSet file separate because it has a distinct build.
#   tests/mutation.sh: CI-only overlays prove the p-85.16 health and handshake
#     contracts against their behavioral Go suites.
#   files/lua/z2k-modern-core.lua + files/lua/z2k-state-persist.lua +
#     webpanel/cgi/actions.sh: p-85.16 restores independent yt_tcp host state.
#   docs/UPSTREAM-CONTRACTS.md + docs/UPSTREAM-SYNC.md + ledger/helper/test:
#     contract matrix and blob-pinned normative-doc sync gate.
...
# graphify-out is local generated analysis output, not an upstream payload or
# adapter source. Preserve it in the worktree, but keep it outside this boundary.
_all="$(printf '%s\n%s\n' "$_changed" "$_untracked" | sed '/^[[:space:]]*$/d' | grep -v '^graphify-out/' | sort -u)"
...
if [ -z "$_all" ]; then
    ...
    for _f in $_bad; do
        case "$_f" in
            ...
            files/lua/z2k-modern-core.lua|files/lua/z2k-state-persist.lua|tests/test_profile_observation.lua|tests/test_webpanel_state.sh|z2k-warpd/internal/transport/wg/bind.go|z2k-warpd/internal/engine/netsetup_test.go|docs/UPSTREAM-CONTRACTS.md|docs/UPSTREAM-SYNC.md|docs/UPSTREAM-SYNC.tsv|scripts/openwrt/audit-upstream-docs.sh|tests/openwrt/test_ow_upstream_docs_sync.sh) continue ;;
            tests/mutation.sh|z2k-warpd/internal/domainroute/*_test.go|z2k-warpd/internal/edgepick/*_test.go|docs/UPSTREAM-TEST-INVENTORY.md) continue ;;
            tests/test_build_matrix.sh|tests/test_update_content_verification.sh|tests/test_warp_games.sh|tests/test_warp_script.sh|tests/test_warp_missing_not_an_error.sh|tests/test_warp_register_retry.sh|z2k-warpd/internal/engine/engine_test.go|z2k-warpd/internal/transport/wg/memory_test.go|z2k-warpd/internal/transport/wg/bind_test.go) continue ;;
