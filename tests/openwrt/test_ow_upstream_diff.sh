...
ALLOWLIST="$ALLOWLIST z2k-warpd/internal/domainroute/nft_pairset.go z2k-warpd/internal/domainroute/nft_pairset_test.go z2k-warpd/openwrt-overlay/overlay.json z2k-warpd/openwrt-overlay/ipset.go z2k-warpd/openwrt-overlay/go.mod tests/test_build_matrix.sh"
...
if [ -z "$_all" ]; then
    ...
    for _f in $_bad; do
        case "$_f" in
            ...
            tests/test_build_matrix.sh) continue ;;
