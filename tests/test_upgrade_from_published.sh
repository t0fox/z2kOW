...
#
# Первый прогон на новом источнике обязан отбросить ETag старого форка и
# скачать тела заново; сохранённый .txt виден до удачной замены. И заодно
# всплывает мусор, который опубликованная ревизия собрать не
...
env -i PATH="$Z2K_TEST_PATH" HOME="$TMP" TMPDIR="$TMP" \
    OPT="$OPT" UL="$UL" SB="$TMP" Z2K_UL_SOURCE_ONLY=1 "$Z2K_TEST_SH" -c '
        ...
        update_warp_game_list
    ' >/dev/null 2>&1
eq "ночной прогон сменил источник и скачал два новых тела" "2" \
   "$(wc -l < "$TMP/nightly-dl.log" 2>/dev/null | tr -d ' ')"
eq "…и пересобрал список из нового кеша" "5.6.7.8" \
   "$(head -1 "$OPT/lists/warp/games/Steam.txt" 2>/dev/null)"
eq "…пометил новый источник" "https://raw.githubusercontent.com/YOZH3G/ru-gaming-blocklist/main" \
   "$(cat "$OPT/lists/warp/.games-source" 2>/dev/null)"
